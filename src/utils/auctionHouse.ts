...
export const getAuctionHouseByAddress = async (
  ...
  const mtlyHouse = await MtlyAuctionHouse.fromAccountAddress(
    connection,
    auctionHouse,
  );
  return mtlyHouse;
...
export const getAuctionHouse = async (
  ...
  try {
    ...
    const rewardCenterAccounts = await connection.getProgramAccounts(
      PROGRAM_ID,
      {
        filters: [
          {
    ...
    if (rewardCenterAccounts.length > 0) {
      rewardCenter = RewardCenter.deserialize(
        rewardCenterAccounts[0].account.data,
        0,
      )[0];
      auctionHouse.rewardCenter = {
        ...
        sellerRewardPayoutBasisPoints:
          rewardCenter.rewardRules.sellerRewardPayoutBasisPoints,
